...
5. Walk [waves 0–9](waves/index.md), then [observability](observability.md) (wave **10**) or delete `applications/kube-prometheus-stack.yaml`. Delete Application files you do not want. Stop at each **Validation** block — the next wave will not paper over a failed check. Upstream docs for every pin: [versions](versions.md).
